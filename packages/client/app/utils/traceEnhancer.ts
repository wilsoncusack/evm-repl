...
import {
  ...
  createPCToSourceMap,
  extractFunctionRanges,
  parseSourceMaps,
} from "./sourceMapParser";
 ...
 */
export function enhanceTracesWithOpcodes(
        ...
        if (pc !== undefined) {
          ...
          if (sourceInfo) {
            ...
            step.sourceInfo = {
              ...
              isOutOfBounds:
                sourceInfo.offset < 0 ||
                sourceInfo.offset > (sourceFiles[filePath]?.length || 0),
              isDuplicate,
            };
      ...
      if (!foundFunction) {
        ...
        if (functionRanges[filePath]) {
          ...
          for (const fnRange of functionRanges[filePath]) {
            ...
            if (
              stepLine >= fnRange.line - 1 &&
              stepLine <= fnRange.endLine + 1
            ) {
...
export function buildSourceTraceMapping(
  ...
  if (compilationResult.contracts) {
    ...
    if (firstContract && compilationResult.contracts[firstContract][0]) {
      bytecode =
        compilationResult.contracts[firstContract][0].contract.evm
  ...
  const enhancedTraces = enhanceTracesWithSourceInfo(
    tracesWithOpcodes,
    pcToSource,
    sourceFiles,
  );
  ...
  const functionToSteps = groupStepsByFunction(allSteps, {
    sourceFiles,
    functionRanges,
    pcToSource,
  });
